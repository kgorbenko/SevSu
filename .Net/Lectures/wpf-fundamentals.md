...
приложений) представляет собой язык разметки, используемый для создания
экземпляров объектов .NET. Хотя язык XAML — это технология, которая может быть
применима ко многим различным предметным областям, его главное назначение —
конструирование пользовательских интерфейсов WPF.

Разработчики давно поняли, что создавать сложные, графически насыщенные
приложения намного легче, если отделить графическую часть от лежащего в основе
кода. Таким образом, художники могут заниматься графикой, а разработчики —
кодом. Обе части могут проектироваться и совершенствоваться по отдельности, без
проблем, связанных с множеством версий.

При проектировании WPF-приложения в Visual Studio создаваемое окно не
транслируется в код. Вместо этого оно сериализуется в набор дескрипторов XAML.
После запуска приложения эти дескрипторы используются для генерации объектов,
составляющих пользовательский интерфейс.

XAML - диалект XML с добавлением ряда правил, относящихся к элементам, атрибутам
и их отображению на объекты, их свойства и значения свойств (помимо всего
прочего). И, хотя форматы, основанные на XML, гибки и легко переносимы на другие
инструменты и платформы, они не всегда являются наиболее эффективным выбором.
XML задуман как непротиворечивый, читабельный и прямолинейный, но не компактный
формат.

В WPF этот недостаток преодолен посредством BAML (Binary Application Markup
Language — двоичный язык разметки приложений). BAML — это не что иное, как
двоичное представление XAML. Когда вы компилируете приложение WPF в Visual
Studio, все файлы XAML преобразуются в код BAML, и этот код BAML затем
встраивается в виде ресурса в финальную сборку DLL или ЕХЕ.И код BAML не только
существенно меньше, но он также оптимизирован, чтобы быстрее интерпретироваться
во время выполнения. 
...
* Каждый элемент в документе XAML отображается на экземпляр класса .NET. Имя
  элемента в точности соответствует имени класса. Например, элемент `<Button>`
  сообщает WPF, что должен быть создан объект `Button`. Ниже приведено объявление
  кнопки в XAML (XAML-документ не полный) и эквивалентный ему код на C#.
  ...
  Результат эквивалентен - кнопка, заполняющая все окно:

  ![Кнопка](images/stretch-button-example.png?raw=true)

* Как и любой XML-документ, код XAML допускает вложение одного элемента внутрь
  другого. Как будет показано, XAML предоставляет каждому классу гибкость в
  принятии решения относительно того, как справиться с такой ситуацией. Однако
  вложение обычно является способом выразить включение (containment). Другими
  словами, если вы видите элемент `Button` внутри элемента `Grid`, то
  пользовательский интерфейс, возможно, включает `Grid`, содержащий внутри себя
  `Button`.
  ...
  ![Кнопка](images/containment-example.png?raw=true)

* Свойства каждого класса можно устанавливать через атрибуты. Тем не менее, в
  некоторых ситуациях атрибуты не достаточно мощны, чтобы справиться с этой
  работой. В этих случаях понадобятся вложенные дескрипторы со специальным
  синтаксисом.

### Структура XAML-документа

Рассмотрим структуру рассмотренного ранее XAML-документа полностью:
...
Документ содержит несколько элементов — элемент верхнего уровня Window,
который представляет все окно. Хотя можно использовать любой элемент верхнего уровня,
приложение WPF полагается только на несколько из них: 

* **Window**;

* **Page** (похож на `Window`, но используется для приложений с возможностью
  навигации).

* **Application** Определяет ресурсы приложения и начальные установки.

Как и во всех документах XML, может существовать только один элемент верхнего
уровня. В предыдущем примере это означает, что закрытие элемента `Window`
дескриптором `</Window>` завершает документ. Никакое дополнительное содержимое
уже не допускается.

Чтобы анализатор XAML мог сопоставить элементы `<Window>`, `<Grid>` и `<Button>`,
необходимо указать, какие пространства имен используются в документе:
...
`xmlns` — это специализированный атрибут в мире XML, который зарезервирован для
объявления пространств имен. В показанном выше фрагменте кода разметки объявлены
два пространства имен, которые будут присутствовать в каждом создаваемом
документе WPF XAML.

* **http://schemas.microsoft.com/winfx/2006/xaml/presentation** — основное
  пространство имен WFP. Оно охватывает все классы WPF, включая элементы
  управления, которые применяются для построения пользовательских интерфейсов.

* **http://schemas.microsoft.com/winfx/2006/xaml** — пространство имен XAML. Оно
  включает различные служебные свойства XAML, которые позволяют влиять на то,
  как интерпретируется документ. Данное пространство имен отображается на
  префикс х. Это значит, что его можно применять, помещая префикс пространства
  имен перед именем элемента (как в `<х:ИмяЭлемента>`).

> :grey_exclamation: Пространства имен объявляются с помощью атрибутов. Эти
атрибуты могут помещаться внутрь начального дескриптора любого элемента. Однако
согласно принятым соглашениям все пространства имен, которые нужно использовать
в документе, должны быть объявлены в самом первом дескрипторе, как это сделано в
данном примере Как только пространство имен объявлено, оно может использоваться
в любом месте документа.

Атрибуты `Title`, `Height`, `Width` соответствуют соответствующим свойствам в
классе `Window`.

Язык XAML позволяет конструировать пользовательский интерфейс, но для создания
функционирующего приложения необходим способ подключения обработчиков событий.
XAML позволяет легко это сделать с помощью атрибута Class, показанного ниже:
...
Элемент `Grid` используется для компоновки, в нем можно разместить свои элементы
управления (В данном случае `Button`).

### Подключение обработчиков событий

Префикс пространства имен х помещает атрибут `Class` в пространство имен XAML,
что означает более общую часть языка XAML. Фактически атрибут `Class` сообщает
анализатору XAML, чтобы он сгенерировал новый класс с указанным именем. Этот
класс наследуется от класса, именованного элементом XML, автоматически во время
компиляции. Вы можете предоставить часть класса `Window`, которая будет
объединена с автоматически сгенерированной частью этого класса. Указанная вами
часть — блестящий контейнер для кода обработки событий.

> :grey_exclamation: Эта "магия" возможна благодаря средству С#, известному под
названием частичные классы (partial class). Частичные классы позволяют разделить
класс на две или более отдельных части во время разработки, которые соединяются
вместе в скомпилированной сборке.

Среда Visual Studio оказывает помощь, автоматически создавая partial класс,
куда можно поместить код обработки событий. Например, при создании приложения по
имени `TestWPFApp`, содержащего окно по имени `MinWindow` (как в предыдущем
примере), Visual Studio начнет с создания следующего базового каркаса класса:
...
В данный момент класс Windowl не содержит реальной функциональности. Однако он
включает одну важную деталь — конструктор по умолчанию, который вызывает метод
`InitializeComponent()`, когда создается экземпляр класса. Метод `LoadComponent()`
извлекает код BAML (скомпилированный XAML) из сборки и использует его для
построения пользовательского интерфейса.

В классе отделенного кода часто требуется программно манипулировать элементами
управления. Например, необходимо читать либо изменять свойства, а также
присоединять или отсоединять обработчики событий на лету. Чтобы обеспечить такую
возможность, элемент управления должен включать XAML-атрибут `Name`.

Добавим на кнопки `OK` и `Cancel`, использованные ранее, имя и добавим
обработчики события нажатия на эти кнопки в класс `MainWindow`:

`MainWindow.xaml:`

```xml
<Button Content="OK" Grid.Column="0" Name="OkButton" Click="ButtonOk_OnClick" />
<Button Content="Cancel" Grid.Column="1" Name="CancelButton" Click="ButtonCancel_OnClick" />
```
...
При добавлении атрибутов `Name` к элементам `Button`, анализатор XAML добавит
...
которые будут доступны из этого класса.

Запись `Click="ИмяМетодаОбработчикаСобытий"` предполагает, что в C# классе,
соответствующем XAML-документу, будет содержаться метод, соответствующий
сигнатуре делегата `Click`. Если это условие выполняется, то при нажатии на
кнопку будет вызван сооветствующий метод.

В методах-обработчиках `ButtonOk_OnClick` и `ButtonCancel_OnClick` вызывается
свойство `IsEnabled` класса `Button`, которое позволяет заблокировать или
разблокировать кнопку для нажатия. `MessageBox.Show()` позволяет вывести
диалоговое окно с сообщением, которое пользователю будет необходимо подтвердить.

Параметр `object sender` представляет из себя объект, который вызвал событие.
В данном случае можно получить конкретный экземпляр кнопки, используя приведение
типов: `var button = sender as Button` или `var button = (Button) sender`.

Параметр `RoutedEventArgs e` представляет параметры данного события. Класс
содержит свойства, позволяющие:

* Получить или обозначить, было ли собылие обработано: `bool Handled`.

* Получить или обозначить объект, который вызвал событие: `object Source`.

* Получить или обозначить `RoutedEvent` событие, соответствующее экземпляру
  `RoutedEventArgs`: `RoutedEvent RoutedEvent`.

Таким образом, при запуске приложения пользователю будут доступны обе кнопки.
После нажатия на одну из них будет вызван соответствующий обработчик события,
после которого одна из кнопок будет заблокирована и появится сообщение.