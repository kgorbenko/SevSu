...
исследования являются способы создания WEB-приложений на платформе \code{.NET}.
...
\code{ASP.NET Core MVC}, \code{EF Core 5}, \code{Azure Web Service}, \code{KnockoutJs},
СУБД \code{SQL SERVER}; получение навыков поддержки и расширения существующих
...
\subsection{КЛИЕНТСКОЕ ПРИЛОЖЕНИЕ НА KNOCKOUTJS}

\subsubsection{Создание проекта приложения}
...
всех возможным видов JSON. Это выполняет метод \code{GetVersion(JToken token)}. Далее, имея
...
Для этого используется метод \code{Serialize<T>(T dto)} и класс \code{VersionedJsonDto<T>}.
\begin{lstlisting}
...
используемые в промышленной \code{.NET} разработке: \code{ASP.NET Core MVC}, \code{EF Core 5},
\code{Azure Web Service}, \code{KnockoutJs}, СУБД \code{SQL SERVER}.
...
\code{NUnit}, библиотеки моков \code{NSubstitute} в ходе решения задач.

Изучена система учета рабочего времени \code{Agilefant}, используемая на предприятии.
